...
function addPdfHeader(body, completionDate) {
  ...
  const subtitleParagraph = body.appendParagraph('Architectural Review Committee');
...
function addCompletionDate(body, completionDate) {
  // Create a table for completion date field
...
function addSignatureSection(body, formData) {
  ...
  const sigTable = body.appendTable([
    ...
    ['Homeowner Signature', formData.signature]
  ]);
...
function sendHomeownerConfirmation(formData, processedFiles) {
  const subject = `Confirmation: Your ARC Request Submission`;

  const body = `Thank you for submitting your Architectural Review Request.
...
---
Your request has been received and is being reviewed by the Architectural Review Committee. You will be contacted if additional information is needed.`;
