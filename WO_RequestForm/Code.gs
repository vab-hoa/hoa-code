// WO Request Form - Apps Script Backend
...
const CONFIG = {
  ...
  // Drive folder for archiving (HOA Board Documents > WO Request Forms)
...
function testAuthorizations() {
  ...
  try {
    ...
    results.drive = 'FAILED: ' + e.message;
  }
  ...
  try {
    ...
    results.documents = 'FAILED: ' + e.message;
  }
...
function testSendEmail() {
  try {
    Logger.log('Test: Attempting to send email to ' + CONFIG.WO_RECIPIENT);
    GmailApp.sendEmail(
      CONFIG.WO_RECIPIENT,
      'TEST: WO Form Email Test',
      'This is a test email from the WO Request Form.\n\nIf you received this, email is working.'
    );
    Logger.log('Test: Email sent successfully');
    return { success: true, message: 'Test email sent to ' + CONFIG.WO_RECIPIENT };
  } catch (e) {
    ...
    <div class="container">
        ...
        <div class="form-content">
            ...
            <form id="arcForm">
                ...
                <div class="form-group">
                    <label for="description" class="required">Description of Work</label>
                    <textarea id="description" name="description" placeholder="Describe the maintenance or repair work needed..." required></textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="otherInfo" name="otherInfo" placeholder="Any additional details or context..."></textarea>
                </div>

                <div class="form-group">
                    <label for="priority" class="required">Priority You Would Assign This Work</label>
                    <select id="priority" name="priority" required>
                        <option value="">-- Select Priority (1=Low, 10=High) --</option>
...
function addPriorityField(body, formData) {
  const priorityValue = formData.priority + ' / 10';
  const fieldTable = body.appendTable([['Priority Assigned', priorityValue]]);
  ...
  const labelPara = labelCell.appendParagraph('Priority Assigned');
  ...
  const valuePara = valueCell.appendParagraph(priorityValue);
...
function addOtherInfoField(body, formData) {
  const label = body.appendParagraph('Additional Information');
  label.setBold(true);
  label.setFontSize(9);
  ...
  const contentParagraph = body.appendParagraph(formData.otherInfo);
  contentParagraph.setFontSize(9);
  contentParagraph.setLineSpacing(1.15);
  contentParagraph.setSpacingBefore(0);
  contentParagraph.setSpacingAfter(2);
...
function generateWoPdf(formData) {
  ...
  try {
    ...
    addPriorityField(body, formData);

    if (formData.otherInfo && formData.otherInfo.trim()) {
      addOtherInfoField(body, formData);
    }
...
function addPdfHeader(body) {
  ...
  const subtitleParagraph = body.appendParagraph('Maintenance & Repair Requests');
...
function addFormFields(body, formData) {
  const fields = [
    ...
    { label: 'Description of Work', value: formData.description }
  ];
...
function sendSubmissionEmail(formData, pdfBlob, processedFiles) {
  const subject = `VaB Work Order Request — ${formData.name} — ${formData.unitAddress}`;
...
Priority Assigned: ${formData.priority} / 10

Description of Work:
${formData.description}

${formData.otherInfo ? 'Additional Information:\n' + formData.otherInfo + '\n\n' : ''}Supporting Documents:
...
function sendHomeownerConfirmation(formData, processedFiles) {
  const subject = `Confirmation: Your Work Order Request Submission`;
...
---
Your request has been received and is being reviewed. You will be contacted if additional information is needed.`;
